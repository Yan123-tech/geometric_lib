...
### Calculate (Программа для расчёта)

- **Функция `calc(fig, func, size)`**
  - Вычисляет площадь или периметр выбранной фигуры на основе заданных параметров.
  - **Аргументы**:
    - `fig`: название фигуры, одна из: 'circle', 'square'.
    - `func`: функция для вычисления, одна из: 'area', 'perimeter'.
    - `size`: список размеров фигуры (например, радиус для круга или длина стороны для квадрата).
  - **Пример:**
    ```python
    >>> calc('circle', 'area', [5])
...
- **Основной блок программы**
  - Пользователь вводит название фигуры, функцию, а также необходимые размеры для фигуры, после чего программа вычисляет результат.
